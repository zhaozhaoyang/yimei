		...
		for(var i = 0; i < s.length; i++) {
			if(s[i].serviceReady) {
				if(~support_channel.indexOf(s[i].id)) {
					...
					console.log(s[i].description)
					btn.setAttribute('id', s[i].id);
					btn.className = 'mui-table-view-cell mui-radio mui-left mui-hidden';
					btn.innerText = '<img class="paytip fl" src="img/zfw1.png" alt="" /><input name="radio" type="radio" paytype="' + s[i].description + '"><p class="fl">' + s[i].description + '支付</p>';
